...
// Images  → compression + conversion WebP, envoi vers /image/upload
// PDF     → envoi brut vers /raw/upload (préserve le fichier vectoriel)
...
export async function uploadToCloudinary(file) {
  ...
  // ── PDF → /raw/upload (conserve le fichier brut) ──────────────────
  if (isPdf) {
    ...
    // On ajoute le suffixe .pdf si absent pour l'identifier côté admin
    return data.secure_url
  }

  // ── Image → compression + /image/upload ──────────────────────────
